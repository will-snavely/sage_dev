...
def create_sage_theme(args):
    ...
    run_docker_cmd(f"composer create-project roots/sage {theme}", 
                   user="root", workdir=themes_dir, service="dev")
    ...
    run_docker_cmd(f"sed -i 's/{search}/{replace}/g' vite.config.js",
            service="dev", workdir=new_theme_dir)   
    save_bedrock_config({
